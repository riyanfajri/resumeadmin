<div class="form-group row align-items-center" :class="{'has-danger': errors.has('pengalaman'), 'has-success': this.fields.pengalaman && this.fields.pengalaman.valid }">
    <label for="pengalaman" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.wawasan.columns.pengalaman') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.pengalaman" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('pengalaman'), 'form-control-success': this.fields.pengalaman && this.fields.pengalaman.valid}" id="pengalaman" name="pengalaman" placeholder="{{ trans('admin.wawasan.columns.pengalaman') }}">
...
<div class="form-group row align-items-center" :class="{'has-danger': errors.has('date'), 'has-success': this.fields.date && this.fields.date.valid }">
    <label for="date" class="col-form-label text-md-right" :class="isFormLocalized ? 'col-md-4' : 'col-md-2'">{{ trans('admin.wawasan.columns.date') }}</label>
        <div :class="isFormLocalized ? 'col-md-4' : 'col-md-9 col-xl-8'">
        <input type="text" v-model="form.date" v-validate="'required'" @input="validate($event)" class="form-control" :class="{'form-control-danger': errors.has('date'), 'form-control-success': this.fields.date && this.fields.date.valid}" id="date" name="date" placeholder="{{ trans('admin.wawasan.columns.date') }}">
